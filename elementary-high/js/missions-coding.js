...
(function () {
    ...
    function createCodingMissions() {
        return [
            ...
            {
                name: '직선 비행',
                description: '이륙하고, 앞으로 10m 이동 후 착륙하세요!',
                hint: 'drone.takeoff(), drone.moveForward(거리), drone.land() 를 순서대로 사용해보세요.',
                timeLimit: 60,
                ...
                setup: function (scene) {
                    ...
                    // 목표 지점 구체
                    var geo = new THREE.SphereGeometry(0.9, 16, 16);
                    var mat = new THREE.MeshBasicMaterial({ color: 0x44ff88, transparent: true, opacity: 0.7 });
                    var sphere = new THREE.Mesh(geo, mat);
                    sphere.position.set(0, 3, -10);
                    scene.add(sphere);
                    this.collectibles.push(sphere);

                    // 수직 빔
                    var beamGeo = new THREE.CylinderGeometry(0.05, 0.05, 3, 8);
                    var beamMat = new THREE.MeshBasicMaterial({ color: 0x44ff88, transparent: true, opacity: 0.3 });
                    var beam = new THREE.Mesh(beamGeo, beamMat);
                    beam.position.set(0, 1.5, -10);
                    scene.add(beam);
                    this.collectibles.push(beam);
                ...
                frameUpdate: function (state) {
                    // Three.js에서 Z 음수 방향이 forward(-Z)
                    if (!this._reachedForward && state.position.z < -8) {
                        this._reachedForward = true;
                        if (this.collectibles[0]) this.collectibles[0].material.opacity = 1.0;
                    }
                },
                objectives: [
                    {
                        description: '이륙 (2m 이상)',
                        check: function (state) { return state.altitude > 2; }
                    },
                    {
                        description: '앞으로 10m 이동',
                        check: function (state, time, mission) { return mission._reachedForward; }
                    },
                    {
                        description: '안전하게 착륙',
                        check: function (state) { return !state.isFlying && state.altitude < 0.3; }
                    }
                ],
                starCriteria: {
            ...
            {
                name: '사각형 비행',
                description: 'for 반복문을 사용해 사각형 모양으로 비행하세요!',
                hint: 'for 반복문을 사용하세요: for (let i = 0; i < 4; i++) { moveForward + turnRight }',
                timeLimit: 120,
                collectibles: [],
                _extraVisuals: [],
                _currentWaypoint: 0,
                ...
                setup: function (scene) {
                    ...
                    this._targets.forEach(function (pos, i) {
                        ...
                        var mat = new THREE.MeshBasicMaterial({
                            color: colors[i], transparent: true, opacity: i === 0 ? 1.0 : 0.5
                        });
                        var sphere = new THREE.Mesh(geo, mat);
            ...
            {
                name: '고도 센서 비행',
                description: 'getAltitude() 센서로 3개의 목표 높이에 도달하세요!',
                hint: 'drone.getAltitude() 로 현재 높이를 확인하면서 moveUp() 을 조절해보세요.',
                timeLimit: 90,
                collectibles: [],
                _extraVisuals: [],
                _reached3m: false,
                ...
                frameUpdate: function (state) {
                    if (!this._reached3m && state.altitude > 2.5 && state.altitude < 3.5) {
                        ...
                        if (this.collectibles[0]) this.collectibles[0].material.opacity = 0.9;
                    }
                    if (!this._reached6m && state.altitude > 5.5 && state.altitude < 6.5) {
                        this._reached6m = true;
                        if (this.collectibles[1]) this.collectibles[1].material.opacity = 0.9;
                    }
                    if (!this._reached9m && state.altitude > 8.5 && state.altitude < 9.5) {
                        this._reached9m = true;
                        if (this.collectibles[2]) this.collectibles[2].material.opacity = 0.9;
                    }
                },
                objectives: [
                    {
                        description: '3m 높이 도달 (getAltitude 활용)',
                    ...
                    {
                        description: '9m 높이 도달 후 안전 착륙',
                        check: function (s, t, m) { return m._reached9m && !s.isFlying; }
                    }
                ],
                starCriteria: {
                    twoStar: function (time) { return time < 65; },
            ...
            {
                name: '웨이포인트 순찰',
                description: 'flyTo(x,y,z)로 5개 지점을 순서대로 방문하세요!',
                hint: 'await drone.flyTo(x, y, z) 를 사용해 각 좌표로 이동하거나, 배열과 반복문을 활용해보세요.',
                timeLimit: 150,
                collectibles: [],
                _extraVisuals: [],
                _currentTarget: 0,
                ...
                frameUpdate: function (state) {
                    ...
                    var dist = Math.sqrt(dx * dx + dy * dy + dz * dz);
                    if (dist < 3) {
                        if (this.collectibles[this._currentTarget * 2]) {
                            this.collectibles[this._currentTarget * 2].material.opacity = 0.1;
                        }
                        this._currentTarget++;
                        if (this._currentTarget < this._targets.length && this.collectibles[this._currentTarget * 2]) {
                ...
                objectives: [
                    {
                        description: '웨이포인트 1, 2, 3 방문',
                        check: function (s, t, m) { return m._currentTarget >= 3; }
                    },
                    {
                        description: '웨이포인트 4, 5 방문',
                        check: function (s, t, m) { return m._currentTarget >= 5; }
                    },
                    {
                        description: '모든 웨이포인트 방문 후 귀환',
                        check: function (s, t, m) { return m._currentTarget >= 5 && !s.isFlying; }
                    }
                ],
                starCriteria: {
                    twoStar: function (time) { return time < 110; },
            ...
            {
                name: '거리 센서 탐색',
                description: 'getDistanceTo()로 숨겨진 착륙 패드를 찾아 착륙하세요!',
                hint: 'drone.getDistanceTo(x, z) 로 목표까지 거리를 확인하세요. 패드 좌표: (15, _, 15)',
                timeLimit: 120,
                collectibles: [],
                _extraVisuals: [],
                _reachedAlt: false,
                ...
                setup: function (scene) {
                    ...
                    var spriteMat = new THREE.SpriteMaterial({ map: texture, transparent: true });
                    var sprite = new THREE.Sprite(spriteMat);
                    sprite.position.set(this._padPosition.x, 0.5, this._padPosition.z);
                ...
                frameUpdate: function (state) {
                    if (!this._reachedAlt && state.altitude > 4.5) {
                        this._reachedAlt = true;
                        if (this.collectibles[0]) this.collectibles[0].material.opacity = 0.9;
                    }
                    var dx = state.position.x - this._padPosition.x;
                    var dz = state.position.z - this._padPosition.z;
                    var dist = Math.sqrt(dx * dx + dz * dz);
                    if (!this._foundPad && dist < 5 && state.altitude > 1) {
                        this._foundPad = true;
                    }
                ...
                objectives: [
                    {
                        description: '5m 고도 비행',
                        check: function (s, t, m) { return m._reachedAlt; }
                    },
                    {
                        description: '패드 5m 이내 접근',
            ...
            {
                name: '자율비행 종합 챌린지',
                description: '모든 코딩 기술을 종합해 완벽한 자율비행을 완성하세요!',
                hint: 'flyTo(), followPath(), getAltitude() 등 배운 모든 API를 활용해보세요!',
                timeLimit: 180,
                collectibles: [],
                _extraVisuals: [],
                _waypointsVisited: 0,
                ...
                frameUpdate: function (state) {
                    // 웨이포인트 체크 (collectibles 0~7, 짝수 인덱스가 구체)
                    for (var i = 0; i < 4; i++) {
                        var sphere = this.collectibles[i * 2];
                        if (!sphere || sphere.userData.visited) continue;
                    ...
                    // 링 체크 (collectibles 8~9)
                    for (var j = 8; j < 10; j++) {
                        var ring = this.collectibles[j];
                        if (!ring || ring.userData.passed) continue;
                        var rdx = state.position.x - ring.position.x;
                        var rdy = state.position.y - ring.position.y;
                        var rdz = state.position.z - ring.position.z;
                ...
                starCriteria: {
                    twoStar: function (time) { return time < 140; },
                    threeStar: function (time) { return time < 90; }
                }
            }
        ];
    }
